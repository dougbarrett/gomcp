...
// TestNewAddComponentResult tests creating an add component result.
func TestNewAddComponentResult(t *testing.T) {
	added := []string{"button", "input", "card"}
	skipped := []string{"modal"}

	result := NewAddComponentResult(added, skipped)

	if !result.Success {
		t.Error("Success should be true")
	}
	if result.Message != "Components added successfully" {
		t.Errorf("Message = %q, want %q", result.Message, "Components added successfully")
	}
	if len(result.ComponentsAdded) != 3 {
		t.Errorf("len(ComponentsAdded) = %d, want 3", len(result.ComponentsAdded))
	}
	if len(result.ComponentsSkipped) != 1 {
		t.Errorf("len(ComponentsSkipped) = %d, want 1", len(result.ComponentsSkipped))
	}
...
// TestNewAddComponentResult_AllNew tests with all new components.
func TestNewAddComponentResult_AllNew(t *testing.T) {
	result := NewAddComponentResult([]string{"a", "b"}, nil)

	if !result.Success {
		t.Error("Success should be true")
	}
	if len(result.ComponentsAdded) != 2 {
		t.Errorf("len(ComponentsAdded) = %d, want 2", len(result.ComponentsAdded))
	}
	if result.ComponentsSkipped != nil {
		t.Error("ComponentsSkipped should be nil")
	}
}

// TestNewAddComponentResult_AllSkipped tests with all skipped components.
func TestNewAddComponentResult_AllSkipped(t *testing.T) {
	result := NewAddComponentResult(nil, []string{"existing"})

	if !result.Success {
		t.Error("Success should be true")
	}
	if result.ComponentsAdded != nil {
		t.Error("ComponentsAdded should be nil")
	}
	if len(result.ComponentsSkipped) != 1 {
		t.Errorf("len(ComponentsSkipped) = %d, want 1", len(result.ComponentsSkipped))
	}
...
// TestNewAddComponentError tests creating an error add component result.
func TestNewAddComponentError(t *testing.T) {
	errors := []string{"button: permission denied", "input: file not found"}

	result := NewAddComponentError("Failed to add components", errors)

	if result.Success {
		t.Error("Success should be false")
	}
	if result.Message != "Failed to add components" {
		t.Errorf("Message = %q, want %q", result.Message, "Failed to add components")
	}
	if len(result.Errors) != 2 {
		t.Errorf("len(Errors) = %d, want 2", len(result.Errors))
	}
	if result.ComponentsAdded != nil {
		t.Error("ComponentsAdded should be nil on error")
	}
}

// TestNewAddComponentError_NoErrors tests error result with nil errors.
func TestNewAddComponentError_NoErrors(t *testing.T) {
	result := NewAddComponentError("General failure", nil)
